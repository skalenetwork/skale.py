...
from tests.constants import DEFAULT_NODE_ID, DEFAULT_NODE_NAME, NOT_EXISTING_ID
...
def test_get(skale: SkaleManager, nodes, node_wallets):
...
def test_get_with_change_ip(skale: SkaleManager, nodes):
    node = skale.nodes.get_with_change_ip(DEFAULT_NODE_ID)
    assert node['ip_change_ts'] is not None
    assert isinstance(node['ip_change_ts'], int)


def test_wrong_node_id(skale):
    with pytest.raises(InvalidNodeIdError):
        skale.nodes.node_status(NOT_EXISTING_ID)

    with pytest.raises(InvalidNodeIdError):
        skale.nodes.node_finish_time(NOT_EXISTING_ID)


def test_active_node_ids(skale: SkaleManager, nodes):
    nodes_number = skale.nodes.nodes_number()
    active_node_ids = skale.nodes.active_node_ids()
    ...
    # 0 is active status
    assert all([skale.nodes.node_status(node_id) == 0 for node_id in active_node_ids])


def test_active_node_ips(skale: SkaleManager, nodes):
    nodes_number = skale.nodes.nodes_number()

    active_node_ips = skale.nodes.active_node_ips()
...
def test_is_node_name_available(skale: SkaleManager, nodes):
    node = skale.nodes.get_by_name(DEFAULT_NODE_NAME)
...
def test_is_node_ip_available(skale: SkaleManager, nodes):
    node = skale.nodes.get_by_name(DEFAULT_NODE_NAME)
...
def test_node_name_to_index(skale: SkaleManager, nodes):
    node_id = skale.nodes.node_name_to_index(DEFAULT_NODE_NAME)
...
def test_node_public_key(skale: SkaleManager, nodes, node_wallets):
    node_id = skale.nodes.node_name_to_index(DEFAULT_NODE_NAME)
    node_public_key = skale.nodes.node_public_key(node_id)
    assert node_public_key == node_wallets[0].public_key

    with pytest.raises(InvalidNodeIdError):
        skale.nodes.node_public_key(NOT_EXISTING_ID)


def test_node_in_maintenance(skale: SkaleManager, nodes):
    node_id = skale.nodes.node_name_to_index(DEFAULT_NODE_NAME)
    assert skale.nodes.node_status(node_id) == NodeStatus.ACTIVE.value

    try:
        skale.nodes.set_node_in_maintenance(node_id)
        assert skale.nodes.node_status(node_id) == NodeStatus.IN_MAINTENANCE.value
    finally:
        skale.nodes.remove_node_from_in_maintenance(node_id)
    assert skale.nodes.node_status(node_id) == NodeStatus.ACTIVE.value


def test_set_domain_name(skale: SkaleManager, nodes):
    node_id = skale.nodes.node_name_to_index(DEFAULT_NODE_NAME)
...
def test_domain_name(skale: SkaleManager, nodes):
    node_id = skale.nodes.node_name_to_index(DEFAULT_NODE_NAME)
    node = skale.nodes.get(node_id)
    assert node['domain_name'] == DEFAULT_DOMAIN_NAME


def test_get_node_next_reward_date(skale: SkaleManager, nodes):
    node_id = skale.nodes.node_name_to_index(DEFAULT_NODE_NAME)
...
def test_change_ip(skale: SkaleManager, nodes):
    node_id, _ = nodes
...
def test_last_change_ip_time(skale: SkaleManager, nodes):
    node_id = skale.nodes.node_name_to_index(DEFAULT_NODE_NAME)
    new_ip = Helper.ip_to_bytes(generate_random_ip())
    tx = skale.nodes.change_ip(node_id, new_ip, new_ip, wait_for=True, timeout=5)
    change_timestamp = skale.nodes.last_change_ip_time(node_id)
