...
class SChains(BaseContract):
    ...
    def get_schains_internal(self):
        return self.skale.get_contract_by_name('schains_internal')

    @format_fields(FIELDS)
    def get(self, id_):
        schains_internal = self.get_schains_internal()
        res = schains_internal.get_raw(id_)
    ...
    def get_schains_for_owner(self, account):
        schains = []
        list_size = self.get_schains_internal().get_schain_list_size(account)

        for i in range(0, list_size):
            id_ = self.get_schains_internal().get_schain_id_by_index_for_owner(account, i)
            schain = self.get(id_)
    ...
    def get_schains_for_node(self, node_id):
        schains = []
        schain_ids = self.get_schains_internal().get_schain_ids_for_node(node_id)
